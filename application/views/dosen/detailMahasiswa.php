	...
	<div class="container-fluid">
		<div class="card">
				...
				<div class="row">
					...
					<div class="col-md">
						...
						<section id="tabel">
									...
									<form id="myform">
									BAB I <input type="checkbox" name="checkfield" id="g01-01"  onchange="doalert(this)"/></form>
									
									<form id="myform1">
									BAB II <input type="checkbox" name="checkfield" id="g01-01"  onchange="doa(this)"/></form>
									
									<form id="myform2">
									BAB III <input type="checkbox" name="checkfield" id="g01-01"  onchange="doal(this)"/></form>
									...
									<div class="form-row">
										...
										<div class="form-group ml-2">
											<form class="status" id="<?= $p->IDPembimbing;?>" nama="Proposal" method="POST" action="<?= base_url('Dosen/accUsers/'.$u->IDSkripsi.'/Proposal');?>">
												<input type="submit" class="btn<?= $p->IDPembimbing;?> btn btn-outline-primary btn-sm" value="Menyetujui Laporan Akhir" <?= $StaProposal ? 'disabled' : '' ?> >
											</form>
										</div> 
											...
											<div id="Skripsi" class="form-row" <?php if (!$StaProposal) { echo 'style="display: none"';	} ?>>
												<div class="form-group col-md"> 
													<form id="<?= $p->IDPembimbing;?>" class="status" nama="Skripsi" method="POST" action="<?= base_url('Dosen/accUsers/'.$u->IDSkripsi.'/Skripsi');?>">
														<input type="submit" <?= $StaSkripsi ? 'Disabled' : '' ?> class="btn<?= $p->IDPembimbing;?> btn btn-outline-primary btn-sm" value="Menyetujui Seminar" 
														<?php if ($proposal->num_rows() === 2) {
															echo 'disabled';
															if ($StaProposal) {
																$status = 'Lembar Pengesahan telah disetujui mohon tunggu acc dari pembimbing yang lain untuk dapat mengacc KP ';
															} else {
																$status = 'Lembar Pengesahan ini telah disetujui pembimbing yang lain.';
															}
														} else {
						...
						<?php if ($pembimbing) { ?>
							<form id="catatanBimbingan" method="POST" action="<?= base_url('dosen/catatan/'.$u->IDSkripsi.'/'.$u->ID.'/'.$isSkripsi);?>">
								...
								<div class="form-group">
									...
									<small>Upload File  <?= $isSkripsi ?></small>
								</div>
								<div class="form-group">
			...
			<script>
				...
				// untuk mengatur checkbox agar tetap stay saat di refresh
				
					function doalert(checkboxElem) {
					if (checkboxElem.checked) {
						alert("Anda yakin ACC BAB ini ??");
						$("#myform").hide();
				...
				// membuat tampilan chek box di hiden
				// $(document).ready(function() {
  
				// $("#myform").checked(function() {
				// 	alert('Anda Yakin Acc BAB ini ?');
				// 	$(".baris1").hide();
				// })
