...
class Cetak extends CI_Controller {
	...
	public function index()
	{
		...
		$pdf->Cell(0, 0, 'FORMULIR PENGAJUAN JUDUL Kerja Praktek', 0,0,'C');
		$pdf->Ln(15);
		$pdf->SetFont('Times', '', 12);
		$pdf->Cell(0, 0, 'Kepada Yth,', 0,0,'L');
		$pdf->Cell(0, 0, 'Bengkulu,.../.../2018', 0,0,'R');
		$pdf->ln(5);
		$pdf->Cell(0, 0, 'Bapak : Aan Erlansari, M.Kom', 0,0,'L');
		$pdf->ln(5);
		$pdf->Cell(0, 0, 'Ka. Prodi Sistem Informasi', 0,0,'L');
		$pdf->ln(5);
		$pdf->Cell(0, 0, 'Univesitas Bengkulu', 0,0,'L');
		$pdf->ln(5);
		...
		$pdf->ln(5);
		$pdf->MultiCell(0,5,'Berhubung dengan telah selesainya semua persyaratan Akademik & Administrasi / Keuangan dan yang lainnya maka dengan ini saya mengajukan permohonan judul untuk KP pada semester ini.',0,'J');
		$pdf->ln(3);
		...
		$pdf->ln(5);
		$pdf->MultiCell(0,5,'Adapun judul yang saya ajukan adalah judul yang menurut saya bisa diselesaikan tepat waktu dan telah melakukan konsultasi dengan beberapa dosen di Universitas Bengkulu, dan dengan alasan ini saya sekalian mengajukan permohonan untuk dosen Pembimbing KP (Formulir Pengajuan Pembimbing)',0,'J');
		$pdf->ln(5);
		$pdf->MultiCell(0,5,'Demikian isi dari Surat Permohonan ini saya buat, besar harapan saya bapak dapat mengabulkannya dan mengeluarkan SK Pembimbing untuk pelaksanaan KP tersebut. Atas kesedian dan pertimbangan bapak saya ucapkan terimakasih.',0,'J');
		$pdf->ln(10);
	...
	public function kartu($nim)
	{
		...
		if ($data) {
			...
			$pdf->MultiCell(100,7, 'Tidak Ditemukan Catatan Bimbingan Dosen Untuk Kerja Praktek Ini', 0);
		}


		$pdf->Cell(120, 0, '', 0,0,'L');
		$pdf->Cell(0, 35, 'Bengkulu, '.date_indo(date('Y-m-d')), 0,2,'L');
