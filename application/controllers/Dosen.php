...
class Dosen extends CI_Controller
{
	...
	function detailMahasiswa($ID)
	{
		...
		// Mengambil data pembimbing yang sedang melihat KP
		...
		// Array Proposal Berfungsi Untuk Menghitung Proposal KP Yang Di ACC
		...
		$data['isSkripsi'] = $this->M_data->find('pembimbing', $isSkripsi) ? $this->M_data->find('pembimbing', $isSkripsi)->num_rows() === 2 ? 'Skripsi
		' : 'Proposal' : 'Proposal';
	...
	function accUsers($ID, $users)
	{
		...
		foreach ($cek['Pembimbing']->result() as $c) {
			...
			$data['Notifikasi'] =  ' ' . $c->JudulSkripsi . ' Laporan Telah Di ACC';
			$data['Catatan'] = ' Laporan Telah Di ACC Oleh : <br>' . $this->session->userdata('Nama') . ' Sebagai Pembimbing ' . $c->StatusPembimbing;
