...
class Admin extends CI_Controller
{
    ...
    {
        $data['jumlah'] = $this->db->from("users")->get()->num_rows();
        $data['jurusan'] = $this->db->from("jurusan")->get()->num_rows();
        $data['konsen'] = $this->db->from("konsentrasi")->get()->num_rows();
    ...
    public function navigasiUsers($nav)
    {
        ...
        $where = $nav == 'Mahasiswa' ? "Status='Mahasiswa' OR Status='KP'" : "Status='Dosen'";
        if ($nav != 'Settings'&& $nav!="coba") {
    ...
    public function tabelNavigasi($page, $user)
    {
        ...
        if ($user === 'Mahasiswa') {
            $where = "(Status='Mahasiswa' OR Status='KP')";
        } else {
            $where['Status'] = $user;
        }
    ...
    private function sendEmail($email, $nama, $password)
    {
        ...
        $this->email->from('bernatdsitumeang08@gmail.com', 'UNIB');
        $this->email->to($email);

        $this->email->subject('Sistem Informasi Kerja Praktek');
        $this->email->message('Selamat Datang ' . $nama . ' di Universitas Muhadi Setiabudi. Sekarang anda bisa login sistem informasi Kerja Praktek dengan menggunakan ID anda. Password : ' . $password . '  Semoga harimu menyenangkan.');
